...
```bat
install_bridge_shortcut.bat
```

双击后自动在桌面创建 `SayIt AI Bridge.lnk`，以后双击即可启动。
...
```bash
py -3 tools/agent_bridge/bridge.py
```

### 启动脚本说明

`start_bridge.bat` 会自动：

1. 定位仓库根目录
2. 验证 `.git` 和 `tools/agent_bridge/bridge.py` 存在
3. 自动寻找可用 Python（优先顺序：`py -3` → `python` → Python 3.12 路径）
4. 启动桥梁
5. 启动失败时显示完整错误并暂停（不会闪退）
6. 通过 `pause` 保持窗口（退出后按任意键关闭）
...
| `start_bridge.bat` | ✅ 提交 | 启动脚本（纯 ASCII/CRLF） |
| `install_bridge_shortcut.bat` | ✅ 提交 | 快捷方式安装入口 |
| `tools/agent_bridge/install_shortcut.ps1` | ✅ 提交 | 桌面快捷方式安装脚本 |
